...
public class GuiWebBrowser {
	...
	private String homePage = "http://www.arip.co.th";
		...
		browser.addLoadListener(new LoadAdapter() {
			...
			public void onStartLoadingFrame(StartLoadingEvent event) {
				...
				if(!url.equalsIgnoreCase("about:blank")) {
					...
					gui.log(tabIndex, "GET URL : " + (url.length() > 50 ? url.substring(0, 40) + "..." : url));
				}
				if(event.isMainFrame()) {
					handleUrl(url, false);
					threadTitle = new Thread(new Runnable() {
						@Override
						public void run() {
			...
			public void onFinishLoadingFrame(FinishLoadingEvent event) {
				...
				if(!url.equalsIgnoreCase("about:blank")) {
					addressBar.setText(url);
					gui.log(tabIndex, "Finish URL : " + (url.length() > 50 ? url.substring(0, 40) + "..." : url));
				}
				if(event.isMainFrame()) {
					handleUrl(url, false);
				}
	...
	private void initGui() {
		...
		browser.loadURL("http://about:blank");
		addressBar.setText("http://about:blank");	
	}
